...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: window.location.origin // Automatically redirects back to your app
      }
    })
    if (error) alert(error.message)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-20 font-sans">
      ...
      <div className="bg-white p-4 shadow-sm sticky top-0 z-50 flex justify-between items-center">
        ...
        <div className="flex gap-2 items-center">
          <button onClick={() => user ? handleLogout() : setShowAuthModal(true)} 
            ...
            {user ? (user.user_metadata?.avatar_url ? <img src={user.user_metadata.avatar_url} className="w-full h-full rounded-full" /> : user.email[0].toUpperCase()) : '👤'}
          </button>
          {isAdmin ? (
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-xl p-6 relative">
            ...
            <div className="space-y-3">
              ...
              <button onClick={handleGoogleLogin} className="w-full bg-white text-gray-700 border border-gray-300 py-3 rounded font-bold flex items-center justify-center gap-2 hover:bg-gray-50">
      ...
      {currentScreen === 'HOME' && (
        <div className="p-4">
          ...
          <div onClick={() => setCurrentScreen('SAVED')} className="bg-pink-100 border border-pink-200 p-4 rounded-xl mb-6 flex items-center justify-between cursor-pointer active:scale-95 transition">
            <div className="flex items-center gap-3"><span className="text-2xl">❤️</span><div><h3 className="font-bold text-pink-800">Saved Questions</h3><p className="text-xs text-pink-600">{bookmarks.length} saved</p></div></div><span className="text-pink-400">➔</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {exams.map(exam => {
              ...
              return (
                <div key={exam.id} onClick={() => handleExamSelect(exam)} className="relative bg-white p-6 rounded-xl shadow border border-gray-100 flex flex-col items-center gap-3 active:scale-95 transition cursor-pointer overflow-hidden">
                  <img src={exam.icon_url} className="w-12 h-12 object-contain" alt="icon" /><span className="font-bold text-gray-800 text-center">{exam.name}</span>
      ...
      {currentScreen === 'SUBJECT_SELECT' && (
        <div className="p-4 space-y-3">
          ...
          {selectedExam.subjects.map(subject => (
            <div key={subject} onClick={() => handleSubjectSelect(subject)} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center active:bg-blue-50 cursor-pointer">
              <span className="font-bold text-gray-800">{subject}</span><span className="text-gray-300">➔</span>
            </div>
          ))}
        </div>
      )}
      ...
      {currentScreen === 'CHAPTER_SELECT' && (
        <div className="p-4 space-y-3">
          <div className="mb-4"><h2 className="text-sm text-gray-500">{selectedExam.name} &gt; {selectedSubject}</h2><h1 className="text-2xl font-bold text-gray-800">Select Chapter</h1></div>
          {getChapters().map(chap => (
            <div key={chap.name} onClick={() => handleChapterSelect(chap.name)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 active:bg-blue-50 cursor-pointer">
              <div className="flex justify-between items-center mb-2"><h3 className="font-bold text-gray-800">{chap.name}</h3><span className="text-blue-500 text-xs font-bold bg-blue-50 px-2 py-1 rounded">{chap.solved}/{chap.total}</span></div>
              <div className="w-full h-1.5 bg-gray-100 rounded-full"><div className="h-full bg-green-500 rounded-full transition-all duration-500" style={{width: `${chap.percent}%`}}></div></div>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="p-4 grid gap-4 md:grid-cols-2">
          {getVisibleQuestions().map((q, i) => (
            <div key={q.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 relative">
               {isAdmin && (<div className="absolute top-4 right-4 flex gap-2 z-10"><button onClick={() => { setEditingQId(q.id); setNewQ({ ...q, exam_id: q.exam_id, text: q.question_text, opA: q.option_a, opB: q.option_b, opC: q.option_c, opD: q.option_d, correct: q.correct_option, chapter: q.chapter, yearTag: q.exam_year || '', solution: q.solution_text || '', existingImage: q.image_url }); setShowAddQForm(true) }} className="text-blue-400 bg-white p-1 rounded shadow">✏️</button><button onClick={() => handleDeleteQ(q.id)} className="text-red-400 bg-white p-1 rounded shadow">🗑️</button></div>)}
               <div className="mb-3 flex justify-between items-start pr-16">
                 <div><div className="text-xs text-gray-400 font-bold uppercase tracking-wide mb-1">{q.chapter}</div><span className={`text-xs px-2 py-0.5 rounded border ${q.difficulty === 'Easy' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-orange-50 text-orange-600 border-orange-200'}`}>{q.difficulty}</span></div>
                 <div className="flex gap-3"><button onClick={() => toggleBookmark(q.id)} className="transition active:scale-125">{bookmarks.includes(q.id) ? '❤️' : '🤍'}</button>{selectedAnswers[q.id] && (<button onClick={() => handleResetQuestion(q.id)} className="text-gray-400 hover:text-blue-600 transition">🔄</button>)}</div>
               </div>
               ...
               {['A','B','C','D'].map(key => (<button key={key} onClick={() => { if(!selectedAnswers[q.id]) { setSelectedAnswers({...selectedAnswers, [q.id]: key}); if(q.correct_option === key) markQuestionSolved(q.id) }}} className={`w-full text-left p-3 rounded-lg border mb-2 text-sm ${selectedAnswers[q.id] ? (q.correct_option===key ? 'bg-green-100 border-green-400' : selectedAnswers[q.id]===key ? 'bg-red-100 border-red-400' : 'opacity-50') : 'hover:bg-gray-50'}`}><span className="font-bold mr-2">{key}.</span> {q[`option_${key.toLowerCase()}`]}</button>))}
               {q.exam_year && <div className="mt-2 text-xs text-gray-400 font-medium flex items-center gap-1">📅 {q.exam_year}</div>}
               {selectedAnswers[q.id] && q.solution_text && (<details className="mt-3 group"><summary className="cursor-pointer text-sm font-bold text-blue-600 bg-blue-50 p-2 rounded hover:bg-blue-100 flex justify-between"><span>💡 View Solution</span><span>▼</span></summary><div className="p-3 bg-gray-50 text-gray-700 text-sm border border-gray-200 rounded-b mt-1 whitespace-pre-wrap">{q.solution_text}</div></details>)}
            </div>
          ))}
        </div>
      )}
      
      {/* ADMIN POPUPS (Same as before - kept compact) */}
