...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session?.user) {
        setUser(session.user)
        syncUserProfile(session.user) // <--- Auto-save to 'profiles'
        fetchCloudUserData(session.user.id)
      } else {
        setUser(null)
        loadLocalUserData()
  ...
  const syncUserProfile = async (u) => {
    // Upsert (Insert or Update) user details to public 'profiles' table
  ...
  // --- STANDARD FUNCTIONS (Test Engine, Auth, etc) ---
  ...
  const goBack = () => { if (currentScreen === 'ADMIN_DASHBOARD') setCurrentScreen('HOME'); else if (currentScreen === 'TEST_SETUP') setCurrentScreen('HOME'); else if (currentScreen === 'TEST_RESULT') setCurrentScreen('TEST_SETUP'); else if (currentScreen === 'TEST_ACTIVE') { if(window.confirm("Quit?")) setCurrentScreen('TEST_SETUP') } else if (['PROFILE', 'SETTINGS', 'RESULTS_HISTORY'].includes(currentScreen)) setCurrentScreen('HOME'); else if (['QUESTIONS', 'SAVED'].includes(currentScreen)) setCurrentScreen('CHAPTER_SELECT'); else if (currentScreen === 'CHAPTER_SELECT') setCurrentScreen('SUBJECT_SELECT'); else if (currentScreen === 'SUBJECT_SELECT') setCurrentScreen('HOME') }
  ...
  return (
    <div className={`min-h-screen font-sans pb-20 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
        ...
        <div className="p-4">
           ...
           <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-white p-4 rounded-xl shadow border border-gray-200 text-center">
                 <p className="text-3xl font-black text-blue-600">{dashboardStats.totalUsers}</p>
                 <p className="text-xs text-gray-400 uppercase font-bold">Students</p>
              </div>
              <div className="bg-white p-4 rounded-xl shadow border border-gray-200 text-center">
                 <p className="text-3xl font-black text-green-600">{dashboardStats.totalSolved}</p>
                 <p className="text-xs text-gray-400 uppercase font-bold">Qs Solved</p>
              </div>
              <div className="bg-white p-4 rounded-xl shadow border border-gray-200 text-center">
                 <p className="text-3xl font-black text-purple-600">{dashboardStats.totalTests}</p>
                 <p className="text-xs text-gray-400 uppercase font-bold">Tests Taken</p>
              </div>
           </div>
              ...
              <table className="w-full text-sm text-left">
                 <thead className="bg-gray-50 text-gray-500 font-bold border-b">
                    <tr>
                       ...
                       <th className="p-3 text-center">Tests</th>
                       ...
                       <tr key={u.id} className="border-b last:border-0 hover:bg-gray-50">
                          <td className="p-3 flex items-center gap-2">
                             <div className="w-8 h-8 rounded-full bg-gray-200 overflow-hidden">
                                {u.avatar_url ? <img src={u.avatar_url} className="w-full h-full object-cover"/> : <span className="flex items-center justify-center h-full text-xs font-bold text-gray-500">👤</span>}
                             </div>
                             <div className="truncate max-w-[100px]">
                                <div className="font-bold text-gray-800 truncate">{u.full_name}</div>
                                <div className="text-xs text-gray-400 truncate">{u.email}</div>
                             </div>
                          </td>
                          <td className="p-3 text-center font-bold text-green-600">{u.solvedCount}</td>
                          <td className="p-3 text-center font-bold text-purple-600">{u.testCount}</td>
                          <td className="p-3 text-right text-xs text-gray-400">{new Date(u.last_active_at).toLocaleDateString()}</td>
                       </tr>
                    ))}
                 </tbody>
              </table>
      ...
      {/* REUSED SCREENS from previous steps (Setup, Active, Result, Profile, etc.) are kept but hidden here for brevity. They are included in the full paste above. */}
